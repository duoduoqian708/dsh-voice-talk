...
- **一键通话**：点一下麦克风，全屏通话层应声而起；红键或 `Esc`，随时挂断。
- **边说边听**：回复边生成边朗读，只念正文；信息流与原生一致，思考与工具调用照旧展示。
- **逐句高亮**：念到哪，亮到哪；播报进度逐句点亮，听得见也看得见。
- **静音与收起**：静音只停收录、不挂断；收起只让出画面、不中断。
- **密钥不出本机**：云端密钥只留在本机，浏览器全程接触不到。
...
「说」可用系统语音，免费且离线，也可选千问、讯飞；「听」**只有云端引擎，没有离线识别**，必须配置对应密钥。最省事的方式：**申请一个阿里云百炼的 DashScope API Key，同时填到「说」和「听」的千问引擎**，听说就都通了。
