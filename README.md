...
DeepSeek Harness Web 的语音对话插件：点一下麦克风，全屏通话层起——边说边听，AI 回复边生成边念，只念正文不念代码。通话内可一键静音（不收录、不挂断），右侧信息流与原生同构（推理思考、工具调用全展示，正文完整 markdown 渲染，仅播报正文），播报进度在信息流里逐句高亮，可一键收起聚焦通话。支持系统离线 / 阿里千问 / 讯飞三种语音引擎，密钥不出 host。
