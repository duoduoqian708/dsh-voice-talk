...
通话层在窄屏上也是能用的——左边说话，右边翻记录，控件会自己站成一列：

![手机端通话模式](docs/screenshots/call-mode-mobile.png)

于是，如果你恰好在本地开了一个能远程访问的窗口，就解锁了一种很新的开发姿势：合上电脑，戴上耳机，出门散步。边走边跟它"聊"下一步怎么改，回来时需求聊明白了，代码也改得差不多了。

不用盯着屏幕这件事，颈椎表示十分感动。至于你是在散步、遛狗还是排队买咖啡——它不挑。
